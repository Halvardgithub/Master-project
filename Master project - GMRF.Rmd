...
\eta_t = \mu + x_t.
$$
For the moment we assume constant $\sigma_t$ for all timepoints, and choose some fixed $\sigma_r$ for the random walk.First, lets make the general functions and do some testing.

```{r}
#function to simulate a realization y
sim_non_shocked_gaussian_data <- function(N, mu, sigma_o, sigma_r){
  #N timepoints, mean mu, and standard deviations observations and the RW1
  eta <- mu + Norm_RW1(sigma_r, N)
  y <- sapply(eta, function(r) rnorm(1, mean = r, sd = sigma_o))
  return(y)
}

sim_non_shocked_gaussian_dataframe <- function(N, mu, sigma_o, sigma_r, n, seed = 50){
  set.seed(seed)
  df <- data.frame(matrix(NA, nrow = N, ncol = n))
  for(i in 1:n){
    df[, i] <- sim_non_shocked_gaussian_data(N, mu, sigma_o, sigma_r) 
  }
  return(df)
}

#Visualising some simulated data
test <- sim_non_shocked_gaussian_dataframe(50, 2, 0.001, 0.03, 5)

plot_realizations(test, "Simulated non-shocked Gaussian data with N=50 and 5 realisations", legend = FALSE)
```
...
sigma_r <- 0.03
mu <- 2
NSG_dataframe <- sim_non_shocked_gaussian_dataframe(N, mu, sigma_o, sigma_r, n)
NSG_dataframe$t <- 1:N

Model_eval_NSG <- data.frame(matrix(NA, nrow = n, ncol = 2)) # need to increase 2 later
colnames(Model_eval_NSG) <- c("DIC_DS", "CPO_DS")
...
for(i in 1:n){#iterate over each simulated realization
  ...
  Model_eval_NSG[i, ] <- c(res_DS$dic$dic, res_DS$cpo$cpo)
}
...
NSG_data <- sim_non_shocked_gaussian_data(N, mu, sigma_o, sigma_r)
...
plot_df <- data.frame(matrix(c(NSG_data, preds), ncol = 2))
...
Sprøsmål:
Burde RW'en summerere til en når jeg simulerer dataen? For å garantere konstant mean over alle realisatsjonene.
Hvordan simulere shocked data?

To do:
Make use of the plot function everywhere
Implement adaptive RW, maybe look at github og wakefield et al
Choose model criteria
Write about INLA

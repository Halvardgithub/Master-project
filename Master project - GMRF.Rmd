...
sim_non_shocked_dataframe <- function(E, mu, sigma, T, n, seed = 42){
  set.seed(seed)
  df <- data.frame(matrix(NA, nrow = T, ncol = n))
...
test <- sim_non_shocked_dataframe(1, 2, 0.2, 100, 5)
...
test_data <- data.frame(matrix(c(test[, 2], 1:100), nrow = 100, ncol = 2)) # should make the 100 more general, like T
colnames(test_data) <- c("y", "time")

res <- inla(formula, family = "poisson", data = test_data)
plot(res)
summary(res)
```

Noe som er litt feil her, må kanskje inkludere E på et vis, tydeligvis mer enn det som er feil, kan hende dataen er for sporadisk
...
Also need some criteria to compare the success of the models.
...
RW2 with unobserved points in the middel
```{r}
# Generate data set

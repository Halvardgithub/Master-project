...
A common model criteria is the root mean square error, or RMSE. We define a function to calculate this below. The lower RMSE the better.
...
plot(res_M$summary.fitted.values$mean, res_I$summary.fitted.values$mean, main = "Comparison of the mean", xlab = "Manual RW1", ylab = "INlA's RW1")
abline(0, 1)
plot(res_M$summary.fitted.values$sd, res_I$summary.fitted.values$sd, main = "Comparison of the standard deviation", xlab = "Manual RW1", ylab = "INlA's RW1")
abline(0, 1)
```
...
figure_list
```
We observe that the standard RW1 struggles a lot with the first and third realization, and the rest of the plots seem nice.Now, lets do some rigorous testing for the entire dataframes form earlier and evaluate them by the chosen model criterias, RMSE and LS. First lets compare the models for non-shocked Gaussian data.
```{r}
...
plot_eval_NS <- ggarrange(plot_RMSE_RW1_NS, plot_RMSE_ARW1_NS,plot_LS_RW1_NS, plot_LS_ARW1_NS, ncol = 2, nrow = 2)
annotate_figure(plot_eval_NS, top = text_grob("Model evaluation for non-shocked Gaussian data")) #can add more parameters like size and color...
```

Lets now test for shocked data.
```{r}
...
plot_eval_NS <- ggarrange(plot_RMSE_RW1_S, plot_RMSE_ARW1_S,plot_LS_RW1_S, plot_LS_ARW1_S, ncol = 2, nrow = 2)
annotate_figure(plot_eval_NS, top = text_grob("Model evaluation for shocked Gaussian data")) #can add more parameters like size and color...
```



check:
maybe weird axis on line 633

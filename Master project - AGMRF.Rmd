  ...
  theta = NULL)
{
  ...
  log.prior <- function() {#default: shape = 1, rate = 0.00005
    p <- interpret_theta()
    prior <- inla.pc.dprec(p$tau1, u = 1, alpha = 0.01) + theta[1L]+
          inla.pc.dprec(p$tau2, u = 1, alpha = 0.01) + theta[2L]
    
    #prior <- dgamma(p$tau1, shape = 1, rate = 0.005, log = TRUE) + theta[1L] +
    #dgamma(p$tau2, shape = 1, rate = 0.005, log = TRUE) + theta[2L]
...
Må gjøres:
implementer med unstructured random effect, altså bare en iid gaussian, både i RW1 og i ARW1, kan kanskje bare legge til en f(iid) i hver formel
simulere data og figurer med harmonisk funksjon, med og uten offset.
Present the box plots with 2 in every box, not the difference.Også ha samme y-akse på hver linje når det er tre ved siden av hverandre.

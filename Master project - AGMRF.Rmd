...
#Comparing the models for RW1 - checks the standard prior
```{r}
...
#Comparing the models for ARW1 - pass the R-matrices or not
...
#print(res_ARW1_f$)
```
We see that it is slightly faster when passing the matrices, but not by a lot.
...
abline(0, 1)
```
Here we see that they are clearly quite different, both for mean and standard deviation. This means that the ARW1 model does not automatically satisfy the sum-to-zero constraint, and that we need to add it.

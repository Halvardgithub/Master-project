...
```
When setting mu = 0, that should be the same as enforcing a sum to zero constariant. 
...
```
The INLA with manual RW1 sometimes breaks down, maybe 1 in 4 times, so I am trying to fix a seed to guarantee I get a result. Seems to work everytime with set.seed(46), apparently not consistent now either.
  ...
  theta = NULL)
{
  ...
  #Need to find a good initial value for theta on the log-scale
  initial <- function() {return(c(4, 4))}#I think 4 is default val for precision
...
set.seed(47)
res_M <- inla(formula_M_RW1, family = "gaussian", data = test_data)
...
figure_list[[1]] <-  Comparison_figure
...
### Copy AR1 from a tutoriol to see if it works

```{r}
inla.rgeneric.AR1.model = function(cmd = c("graph", "Q", "mu", "initial", "log.norm.const", "log.prior", "quit"), theta = NULL)
{
envir = parent.env(environment())
...
N <- 50 #is defined further up as well
AR1_model <- inla.rgeneric.define(inla.rgeneric.AR1.model, n = N)


#The INLA formula for a latent model with intercept and user defined AR1
formula_M <- y ~ f(time, model = AR1_model)
                #extraconstr = list(A = matrix(1, nrow = 1, ncol = N), e = 0))
...
plot_M <- plot_realizations(plot_df_M, "Predictions with manually implemented AR1")
...
summary(res_I)
```
The manual AR1 appears to break down every time, so there might have been some updates to INLA which makes this way of defining random effects break down, however the manual RW1 worked sometimes, although the returned model performed quite bad and no where close to INLA's implementation. 


Sprøsmål:
Should I scale the R-matrix when defining the precision matrix Q in the RW1? By geometric variance and so on 
When a return numeric(0) for the mean, will that guarantee that it sums to zero?
...
Should pass, for instance the Q, to the INLA function to avoid computing it inside the function every the model is trained for new data.

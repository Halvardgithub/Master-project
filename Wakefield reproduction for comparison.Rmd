...
```{r}
# mod_eval_W_testing <- function(df_y, df_eta, P) {
#   
#   eval_df <- data.frame(matrix(NA, nrow = 1, ncol = 4))
#   colnames(eval_df) <- c("RMSE_RW1", "LS_RW1", "RMSE_ARW1", "LS_ARW1")
# 
#   
#   test_data <- df_y
...
#                                   list(initial = log(P), fixed = TRUE))))
#   LS_RW1 <- -mean(log(res_RW1$cpo$cpo))
#   RMSE_RW1 <- RMSE(df_eta[, 1], res_RW1$summary.fitted.values$mean)
#   
#   res_ARW1 <- inla(formula_ARW1_W, family = "gaussian", data = test_data,
#                    control.compute = list(cpo = TRUE),
#                    control.family = list(hyper = list(prec = 
#                                   list(initial = log(P), fixed = TRUE))))
#   LS_ARW1 <- -mean(log(res_ARW1$cpo$cpo))
#   RMSE_ARW1 <- RMSE(df_eta[, 1], res_ARW1$summary.fitted.values$mean)
#   
#   eval_df[1, ] <- c(RMSE_RW1, LS_RW1, RMSE_ARW1, LS_ARW1)
#   
#   return(list(eval_df, res_RW1, res_ARW1))
# }
# 
# result <- mod_eval_W_testing(data_w_flat_75_CT[[1]][, c(1, 101, 102)], data_w_flat_75_CT[[2]][, 1], 75)
```
